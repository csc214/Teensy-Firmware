...
#define N25DELAY asm volatile( "nop\nnop\nnop\nnop\nnop\n" )
...
#define C_HIGH GPIOD_PSOR = 0b100
...
void hshift(int flag) {
  ...
  for (int b = 0; b < WIDTH; b++) {
    ...
    NXDELAY(500);               //t_C
    HC_TOGGLE;
    delayMicroseconds(1.5);       //t_sd
    if (flag){
        aread = analogRead(video);
        *img_end++ = (aread >> 8);
        *img_end++ = aread & 0xff;
        }
    }//end row shifting
  if (flag) Serial.write((byte*)img, WIDTH * 2); // Twice the height because every pixel is a 16-bit number
}
